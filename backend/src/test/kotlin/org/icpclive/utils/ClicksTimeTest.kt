...
class ClicksTimeTest {
    ...
    fun parseRelativeTimeTest() {
        for (pp in relSigns) for (hh in relHours) for (mm in relMinutes) for (ss in relSeconds) {
            ...
            val result: Long = parseRelativeTime(testRelTime).toDouble(DurationUnit.MILLISECONDS).roundToLong()
    ...
    fun durationJsonSerialisation() {
        ...
        for (pp in relSigns) for (hh in relHours) for (mm in relMinutes) for (ss in relSeconds) {
            ...
            val durationString = formatIso(duration)
            parseRelativeTime(durationString)
