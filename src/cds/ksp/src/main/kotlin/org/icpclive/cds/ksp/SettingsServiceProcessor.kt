...
class SettingsServiceProcessor(val generator: CodeGenerator, val logger: KSPLogger) : SymbolProcessor {
    val allGeneratedFiles = mutableListOf<String>()
    override fun process(resolver: Resolver): List<KSAnnotated> {
        ...
        subTypesOfSettings.forEach {
            ...
            PrintWriter(
                ...
                it.println("import kotlin.reflect.KClass")
                it.println()
                it.println("public class ${className}Provider : CDSSettingsProvider {")
                it.println("  public override val clazz: KClass<*> = ${className}::class")
                it.println("  public override val serializer: KSerializer<*> = serializer<${className}>()")
                it.println("}")
            }
