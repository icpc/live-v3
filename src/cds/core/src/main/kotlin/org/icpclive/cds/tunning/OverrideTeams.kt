...
public data class OverrideTeams(public val rules: Map<TeamId, Override>): TuningRule {
        ...
        val newTeams = mergeOverrides(
            ...
            team.copy(
                ...
                medias = TeamMediaType.entries
                    .associateWith { it -> (override.medias?.get(it) ?: team.medias[it]).orEmpty() + override.extraMedias?.get(it).orEmpty() }
    ...
    public class Override(
        ...
        public val medias: Map<TeamMediaType, @Serializable(with = ListOrSingleElementSerializer::class) List<MediaType>>? = null,
        public val extraMedias: Map<TeamMediaType, @Serializable(with = ListOrSingleElementSerializer::class) List<MediaType>>? = null,
    )
}