package org.icpclive.clics.objects

import org.icpclive.ksp.clics.*

@SinceClics(FeedVersion.`2020_03`)
@EventSerialName("organizations")
public data class Organization(
    @Required override val id: String,
    public val icpcId: String? = null,
    public val name: String? = null,
    public val formalName: String? = null,
    public val country: String? = null,
    public val countryFlag: List<File> = emptyList(),
    @SinceClics(FeedVersion.`2026_01`) public val countrySubdivision: String? = null,
    @SinceClics(FeedVersion.`2026_01`) public val countrySubdivisionFlag: List<File> = emptyList(),
    public val url: String? = null,
    public val twitterHashtag: String? = null,
    @SinceClics(FeedVersion.`2023_06`) public val twitterAccount: String? = null,
    public val location: Location? = null,
    public val logo: List<File> = emptyList()
): ObjectWithId
