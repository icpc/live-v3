...
const getTeamViewSettings = async (teamId: TeamId, media: TeamMediaType) => {
    ...
    const adminSettings: ExternalTeamViewSettings = {
        mediaTypes: [media],
        teamId: teamId,
...
const testTeamViewOneMedia =
    (teamId: TeamId, media: TeamMediaType) =>
    async ({ page }: PlaywrightTestArgs, testInfo: TestInfo) => {
        const widgets = [await getTeamViewSettings(teamId, media)];
        await page.goto(
            `${BACKEND_URL}/overlay?forceWidgets=${encodeURIComponent(JSON.stringify(widgets))}`,
        );

        while (true) {
            await page.waitForTimeout(500);
            const teamViewDisplay = await page
                .locator(".TeamViewContainer")
                .first()
                .evaluate((el) => {
                    return window
                        .getComputedStyle(el)
...
const contestInfo = JSON.parse(
    fs.readFileSync(path.join(__dirname, "contestInfo.json")).toString("utf-8"),
) as ContestInfo;
