...
import { useReloadHandler } from "@admin/services/reloadHandler.ts";
...
> extends AbstractWidgetWithStatus<ObjectStatus<Settings>> {
    ...
    showWithSettings(settings: Settings) {
        return this.apiPost("/show_with_settings", settings).catch(
            this.errorHandler("Failed to show " + this.apiPath),
        );
    }

    setSettings(settings: Settings) {
        return this.apiPost("/", settings).catch(
            this.errorHandler("Failed to set settings " + this.apiPath),
        );
    }
}

export function useSingleWidgetService<Settings extends ObjectSettings>(
    apiPath: string,
) {
