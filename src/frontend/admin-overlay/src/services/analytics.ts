...
import { useWebsocket } from "@admin/utils";
import { BASE_URL_BACKEND, BASE_URL_WS } from "@admin/config";
import {
    AnalyticsEvent,
    AnalyticsMessage,
    ContestInfo,
...
export const useAnalyticsService = () => {
    const [messages, setMessages] = useState<{
        [id: string]: AnalyticsMessage;
    }>({});
    const messagesList = useMemo(
        () =>
            Object.values(messages).sort(
                (a: AnalyticsMessage, b: AnalyticsMessage) =>
                    b.updateTimeUnixMs - a.updateTimeUnixMs,
            ),
        [messages],
    );
                ...
                case AnalyticsEvent.Type.AnalyticsMessageSnapshot:
                    setMessages(() =>
                        event.messages.reduce(
                            (ac, message) => ({ ...ac, [message.id]: message }),
                            {},
                        ),
                    );
                    break;
    ...
    const handleWSMessage = useCallback(
        (e: { data: string }) =>
            processEvent(JSON.parse(e.data) as AnalyticsEvent),
        [processEvent],
    );
    ...
    const deleteAdvertisement = (messageId: string, commentId: string) =>
        apiPost(
            "/" + messageId + `/${commentId}/advertisement`,
            undefined,
            "DELETE",
        );
    ...
    const deleteTickerMessage = (messageId: string, commentId: string) =>
        apiPost(
            "/" + messageId + `/${commentId}/tickerMessage`,
            undefined,
            "DELETE",
        );
