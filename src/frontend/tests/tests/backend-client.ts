...
export class BackendClient {
    ...
    finalizedContestInfo() {
        const contestInfo = new WebSocket(
            `${this.wsURL}/api/overlay/contestInfo`,
        );
        ...
        return new Promise((resolve) => {
            contestInfo.onmessage = (event) => {
                ...
                if (
                    message.status.type === "over" ||
                    message.status.type == "finalized"
                ) {
                    contestInfo.close();
                    resolve(message);
                }
    ...
    async showWidget(widget: WidgetSetting) {
        const showWidget = await this.adminApiContext.post(
            `./${widget.path}/show_with_settings`,
            {
    ...
    async makeFeatured(messageId: string, mediaType: string) {
        const showWidget = await this.adminApiContext.post(
            `./analytics/${messageId}/featuredRun`,
            {
                data: mediaType,
    ...
    async hideFeatured(messageId: string) {
        const showWidget = await this.adminApiContext.delete(
            `./analytics/${messageId}/featuredRun`,
        );
        return showWidget.ok();
    }
